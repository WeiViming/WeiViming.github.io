...
(function($) {
	...
	$.fn.toc = function(options) {
      ...
      let scrollValue = parseInt($("#article_with_toc").scrollTop() + ($(target).offset().top  - settings.scrollOffset) || 0)
        $("#article_with_toc").animate(
		...
		return this.each(function(index1) {
			...
			headlines.each(function(index2, headline) {
				...
				if (settings.strip && shortenedHeadlineText.length > settings.stripAfter) {
					shortenedHeadlineText = shortenedHeadlineText.substring(0, settings.stripAfter) + "...";
				}
				$(this).html('<a id="' + headlineId + '">' + currentHeadlineHTML);
				toc += '<li><a href="javascript:;" data-href="#' + headlineId + '" title="' + currentHeadlineText + '">' + shortenedHeadlineText + '</a></li>';
			});
